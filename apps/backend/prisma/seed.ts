...
async function main() {
  ...
  // Create Genres
  const genreNames = ['Action', 'Drama', 'Sci-Fi', 'Horror', 'Crime', 'Documentary'];
  ...
  for (const m of movies) {
    const content = await prisma.content.create({
      data: {
        ...
        genres: {
          create: m.genre.map((gName) => ({
            genreId: genresMap[gName].id,
          })),
        },
      },
    });
