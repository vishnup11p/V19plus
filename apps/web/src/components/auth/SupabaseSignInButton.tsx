...
export function SupabaseSignInButton() {
  const { supabaseLogin } = useAuthStore();
  ...
  const handleSupabaseSignIn = async () => {
    ...
    if (isNative) {
      toast.error('Google Sign-In is temporarily unavailable inside the mobile app. Please sign up or log in using your email and password, or use our website.', {
        duration: 8000,
      });
      return;
    }
  ...
  return (
    <div className="w-full space-y-3">
      <motion.button
        ...
        className="w-full flex items-center justify-center gap-3 py-3.5 px-5 bg-gradient-to-r from-red-700 to-red-600 hover:from-red-600 hover:to-red-500 text-white rounded-xl font-semibold text-sm transition-all duration-200 disabled:opacity-60 shadow-md shadow-black/30 hover:shadow-lg hover:shadow-red-950/20"
      >
        {loading ? (
          <>
...
function SupabaseIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M72.24 6.72c-2.48-5.36-9.76-5.84-12.88-1.04L4.88 77.2C2.08 81.36 5.04 87.2 10.08 87.2h44.48l-9.04 34.08c-2.48 5.36 9.76 5.84 12.88 1.04l54.48-71.52c2.8-4.16-.16-10-5.2-10H63.2l9.04-34.08z" fill="#FF6B1A" />
    </svg>
  );
}
