...
export function Home() {
  ...
  const filteredFeatured = activeProfile.isKids
    ? featured?.filter((c) => ['G', 'PG', 'U', 'U/A 7+'].includes(c.rating))
    : featured;
  const heroContent = filteredFeatured?.[0];
  ...
  const top10 = activeProfile.isKids
    ? trending?.filter((c) => ['G', 'PG', 'U', 'U/A 7+'].includes(c.rating)).slice(0, 10)
    : trending?.slice(0, 10);
  ...
  return (
    <div className="min-h-screen bg-n-bg animate-fade-in">
      ...
      {filteredFeatured && filteredFeatured.length > 1 && (
        <ContentRow
          title="Featured Titles"
          items={filteredFeatured.slice(1)}
